...
const Brandnew = (props) => {
  ...
  const searchBodyTypes = (filter_id) => {
    router.push({
      pathname: "/brandnew/type/[type]",
      query: { type: filter_id },
    });
  };
  const next = () => {
            ...
            <div className="container">
              ...
              <div className="row brand-row">
                {makes.slice(0, limit).map((make, index) => (
                  <Link
                    href={{
                      pathname: "/brandnew/[brand]",
                      query: { brand: make.trim() },
                    }}
                    className="col"
                    key={index}
                  >
              ...
              <div className="row mt-4">
                <div className="type-container" id="scroll">
                  {types.map((type, index) => (
                    <div
                      className="type-content text-center"
                      key={index}
                      onClick={() => searchBodyTypes(type.filter_id)}
                    >
