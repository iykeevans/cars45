...
export default {
    ...
    getSearch: (searchParam) => `${baseURL}/car/search?make=${searchParam?.make || ""}&model=${searchParam?.model || ""}&year=${searchParam?.year || ""}&minPrice=${searchParam?.minPrice || ""}&maxPrice=${searchParam?.maxPrice || ""}&minYear=${searchParam?.minYear || ""}&maxYear=${searchParam?.maxYear || ""}&transmission=${searchParam?.transmission || ""}&condition=${searchParam?.condition || ""}&grade=${searchParam?.grade || ""}&financeable=${searchParam?.financeable || ""}&type=${searchParam?.type || ""}&page=${searchParam?.page || 0}`,
