...
export default {
    ...
    getSearch: (searchParam)=>`${baseURL}/car/search?make=${searchParam?.make||""}&model=${searchParam?.model||""}&year=${searchParam?.year||""}&minPrice=${searchParam?.minPrice||""}&maxPrice=${searchParam?.maxPrice||""}`,
    getSingleCar: (sku)=>`${baseURL}/car?sku=${sku}`,
    getCity: (city)=>`${baseURL}/location?city=${city}`,
